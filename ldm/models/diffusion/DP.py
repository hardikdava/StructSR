...
def visualize_fea(save_path, fea_img):
    ...
    # im = plt.imshow(fea_img, vmin=0.0, vmax=1.0, cmap='hot', aspect='auto') # Show the image
...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def calculate_variance(self, tensor):
        ...
        variance = torch.mean(squared_diff, dim=(2, 3), keepdim=True)
        return variance
        ...
        for row in range(grid_rows):
            ...
            for col in range(grid_cols):
                ...
                if len(SSIM_list) == lamda:
                    cond_tile_2 = residual_list[0][:, :, input_start_y:input_end_y, input_start_x:input_end_x]
                    cond_list_2.append(cond_tile_2)
                if len(input_list) == batch_size or col == grid_cols-1:
                    input_list = torch.cat(input_list, dim=0)
                    cond_list = torch.cat(cond_list, dim=0)
                    if len(SSIM_list) == lamda:
                        cond_list_2 = torch.cat(cond_list_2, dim=0)
                    ...
                    if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
                        ...
                        if len(SSIM_list) == lamda:
                            struct_cond_input_2 = self.structcond_stage_model(cond_list_2,t_in[:input_list.size(0)])
                            model_out_2 = self.apply_model(input_list, t_in[:input_list.size(0)], c[:input_list.size(0)],
                                                         struct_cond_input_2, return_ids=return_codebook_ids)
                            c1 = max(SSIM_list)
                            c1 = c1**2
    ...
    def _constant_weights(self, tile_width, tile_height, nbatches):
        weights = np.ones((tile_width, tile_height))
        return torch.tile(torch.tensor(weights, device=self.betas.device), (nbatches, self.configs.model.params.channels, 1, 1))
    ...
    def p_sample_loop_canvas(self, cond, struct_cond, shape, return_intermediates=False,
        ...
        for i in iterator:
            ...
            img = self.p_sample_canvas(img, cond, struct_cond, ts,
                                ...
                                tile_size=tile_size, tile_overlap=tile_overlap, batch_size=batch_size, tile_weights=tile_weights,
                                unconditional_conditioning=unconditional_conditioning, unconditional_guidance_scale=unconditional_guidance_scale,
                                reference_sr=reference_sr, reference_lr=reference_lr, reference_step=reference_step, reference_range=reference_range,vq_model=vq_model,im_lq_bs=im_lq_bs,SSIM_list=SSIM_list,residual_list=residual_list,timesteps=timesteps)
            if adain_fea is not None:
                if i < 1:
                    img = adaptive_instance_normalization(img, adain_fea)
            if mask is not None:
        ...
        # import csv
        # csv_file = '/home/algroup/lyc/StableSR/SSIM.csv'
        # # 使用CSV模块打开文件并以追加模式写入数据
        # with open(csv_file, 'a+', newline='') as file:
        #     writer = csv.writer(file)
        #     writer.writerow([str(item) for item in SSIM_list])
