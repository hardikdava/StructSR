...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def p_mean_variance(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        ...
                        reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000],vq_model=None,im_lq_bs=None,SSIM_list=None,residual_list=None,timesteps=None,lamda=None):
        ...
        # set $T_{SAS}$
        lamda = int(timesteps * lamda)

        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            model_out = self.apply_model(x, t_in, c, struct_cond, return_ids=return_codebook_ids)

            # Structure Condition Embedding (SCE)
            if len(SSIM_list) == lamda:
                struct_cond_2 = residual_list[0]
                struct_cond_2 = self.structcond_stage_model(struct_cond_2, t_in)
                model_out_2 = self.apply_model(x, t_in, c,
                                               struct_cond_2, return_ids=return_codebook_ids)
                c1 = max(SSIM_list)
                model_out = (1 - c1) * model_out + c1 * model_out_2
        else:
            x_in = torch.cat([x] * 2)
        ...
        # Structure-Aware Screening (SAS)
        if len(SSIM_list) < lamda:
            #decode the reconstructed image
            _, enc_fea_lq = vq_model.encode(im_lq_bs)
            x_samples = vq_model.decode(x_recon * 1. / self.scale_factor, enc_fea_lq)
            im_sr = torch.clamp((x_samples + 1.0) / 2.0, min=0.0, max=1.0)
            im_sr = im_sr.cpu().numpy().transpose(0, 2, 3, 1) * 255

            # LR image has resized before
            im_lr = torch.clamp((im_lq_bs + 1.0) / 2.0, min=0.0, max=1.0)
            ...
            if max(SSIM_list) == SSIM:

                #update Z_SE
                if len(residual_list) == 0:
                    residual_list.append(x_recon)
                else:
                    residual_list[0] = x_recon
        else:
            # Image Details Embedding (IDE)
            c2 = t[0] / (timesteps - lamda)
            c2 = c2 * max(SSIM_list)
            x_reference_1 = residual_list[0]
            x_recon = c2 * x_reference_1 + (1 - c2) * x_recon
    ...
    def p_mean_variance_canvas(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        ...
                        reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000],vq_model=None,im_lq_bs=None,SSIM_list=None,residual_list=None,timesteps=None):
            ...
            for col in range(grid_cols):
                ...
                if len(input_list) == batch_size or col == grid_cols - 1:
                    ...
                    if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
                        ...
                        model_out = self.apply_model(input_list, t_in[:input_list.size(0)], c[:input_list.size(0)],
                                                     struct_cond_input, return_ids=return_codebook_ids)
                    else:
                        ...
                        e_t_uncond, e_t = self.apply_model(input_list_, t_in_, c_in, struct_cond_input,
                                                           return_ids=False).chunk(2)
                        model_out = e_t_uncond + unconditional_guidance_scale * (e_t - e_t_uncond)
                        return_codebook_ids = False

                    if score_corrector is not None:
                        assert self.parameterization == "eps"
                        model_out = score_corrector.modify_score(self, model_out, input_list, t[:input_list.size(0)],
                                                                 c[:input_list.size(0)], **corrector_kwargs)
        ...
        for row in range(grid_rows):
            for col in range(grid_cols):
                ...
                noise_pred[:, :, input_start_y:input_end_y, input_start_x:input_end_x] += noise_preds[row][
                                                                                              col] * tile_weights
    ...
    def p_sample(self, x, c, struct_cond, t, clip_denoised=False, repeat_noise=False,
        ...
        outputs = self.p_mean_variance(x=x, c=c, struct_cond=struct_cond, t=t, clip_denoised=clip_denoised,
                                       ...
                                       reference_sr=reference_sr, reference_lr=reference_lr, reference_step=reference_step, reference_range=reference_range,vq_model=vq_model,im_lq_bs=im_lq_bs,SSIM_list=SSIM_list,residual_list=residual_list,timesteps=timesteps,lamda=lamda)
        if return_codebook_ids:
            raise DeprecationWarning("Support dropped.")
            model_mean, _, model_log_variance, logits = outputs
        elif return_x0:
    ...
    def p_sample_canvas(self, x, c, struct_cond, t, clip_denoised=False, repeat_noise=False,
                 ...
                 reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000],vq_model=None,im_lq_bs=None,var_min=None,x_reference =None,SSIM_list=None,residual_list=None,timesteps=None):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance_canvas(x=x, c=c, struct_cond=struct_cond, t=t, clip_denoised=clip_denoised,
                                       ...
                                       unconditional_conditioning=unconditional_conditioning, unconditional_guidance_scale=unconditional_guidance_scale,
                                       reference_sr=reference_sr, reference_lr=reference_lr, reference_step=reference_step, reference_range=reference_range,vq_model=vq_model,im_lq_bs=im_lq_bs,var_min=var_min,x_reference = x_reference,SSIM_list=SSIM_list,residual_list=residual_list,timesteps=timesteps)
        if return_codebook_ids:
            raise DeprecationWarning("Support dropped.")
            model_mean, _, model_log_variance, logits = outputs
        elif return_x0:
    ...
    def p_sample_loop(self, cond, struct_cond, shape, return_intermediates=False,
                      ...
                      reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000],vq_model=None,im_lq_bs=None,lamda=None):
    ...
    def p_sample_loop_canvas(self, cond, struct_cond, shape, return_intermediates=False,
                      ...
                      unconditional_conditioning=None, unconditional_guidance_scale=None,
                      reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000],vq_model=None,im_lq_bs=None):
        ...
        for i in iterator:
            ...
            img = self.p_sample_canvas(img, cond, struct_cond, ts,
                                ...
                                tile_size=tile_size, tile_overlap=tile_overlap, batch_size=batch_size, tile_weights=tile_weights,
                                unconditional_conditioning=unconditional_conditioning, unconditional_guidance_scale=unconditional_guidance_scale,
                                reference_sr=reference_sr, reference_lr=reference_lr, reference_step=reference_step, reference_range=reference_range,vq_model=vq_model,im_lq_bs=im_lq_bs,timesteps=timesteps)
    ...
    def sample(self, cond, struct_cond, batch_size=16, return_intermediates=False, x_T=None,
        ...
        return self.p_sample_loop(cond,
                                  ...
                                  unconditional_conditioning=unconditional_conditioning,
                                  unconditional_guidance_scale=unconditional_guidance_scale,
                                  reference_sr=reference_sr, reference_lr=reference_lr, reference_step=reference_step, reference_range=reference_range,vq_model=vq_model,im_lq_bs=im_lq_bs,lamda=lamda)
    ...
    def sample_canvas(self, cond, struct_cond, batch_size=16, return_intermediates=False, x_T=None,
               ...
               unconditional_conditioning=None, unconditional_guidance_scale=None,vq_model=None,im_lq_bs=None, **kwargs):
        ...
        return self.p_sample_loop_canvas(cond,
                                  ...
                                  unconditional_conditioning=unconditional_conditioning, unconditional_guidance_scale=unconditional_guidance_scale, batch_size=batch_size_sample, log_every_t=log_every_t, vq_model=vq_model,im_lq_bs=im_lq_bs)
