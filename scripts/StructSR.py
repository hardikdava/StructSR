...
def main():
	...
	parser.add_argument(
		...
		type=float,
		default=0.3,
		help="Cotrol the timesteps of early inference stage",
	)
	...
	niters = math.ceil(len(init_image_list) / batch_size)
			...
			with model.ema_scope():
				for n in trange(niters, desc="Sampling"):
					...
					for i in range(batch_size):
						init_image.append(init_image_list[n*batch_size+i])
					...
					samples, _ = model.sample(cond=semantic_c, struct_cond=init_latent, batch_size=init_image.size(0), timesteps=opt.ddpm_steps, time_replace=opt.ddpm_steps, x_T=x_T, return_intermediates=True, vq_model=vq_model,im_lq_bs=init_image,lamda=opt.lamda)
